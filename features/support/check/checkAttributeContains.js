...
module.exports = async function(attribute, selector, not, containedValue) {
  ...
  assert.strictEqual(
    attributeValue.includes(containedValue),
    shouldValueBeContained,
    `Expected "${attributeValue}" to ${
      shouldValueBeContained ? 'contain' : 'not contain'
    } "${containedValue}" of element "${selector}" attribute "${attribute}"`,
  )
}
