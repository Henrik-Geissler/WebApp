...
module.exports = async function (screenName, rootDir) {
  ...
  const environment = this.config.environment
    ...
    : '' // Allow for different screenshots per environment
  ...
  if (!(await pathExists(pathRef))) {
    ...
    assert.strictEqual(
      imgCompare.width,
      imgRef.width,
      'Expected screenshot widths to match.',
    )
    assert.strictEqual(
      imgCompare.height,
      imgRef.height,
      'Expected screenshot heights to match.',
    )

    // Compare the images
    const imgDiff = await new PNG({
      width: imgCompare.width,
      height: imgCompare.height,
    })
