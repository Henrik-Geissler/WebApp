Feature: openUrl
  ...
  Scenario: Open a URL in a given step
    Given the url "http://localhost:3000/checkTitle.html" is opened
  ...
  Scenario: Open a URL in a when step using a root URL environment variable
    ...
    Then  I expect the page url is "http://localhost:3000/waitFor.html"

  Scenario: Set the user agent string
    When  I open the url "http://localhost:3000/setUserAgent.html" with user agent "IE6"
    And   I wait for element ".user-agent"
    Then  I expect the element ".user-agent" contains text "IE6"

  Scenario: Emulate a device
    When  I open the url "http://localhost:3000/setUserAgent.html" with device "iPhone X"
    And   I wait for element ".user-agent"
